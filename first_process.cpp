...
int main(int argc, char* argv[])
{
    ...
    log_file << "PID: " << _getpid() << ", counter: " << data->GetValue() << " ,Creating first proccess time: " << c_end_time;
    ReleaseMutex(mut);
#else
    _mutex.lock();
    log_file << "PID: " << getpid() << ", counter: " << data->GetValue() << " ,Creating first proccess time: " << c_end_time;
    _mutex.unlock();
#endif
